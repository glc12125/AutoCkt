...
def gen_data(CIR_YAML, env, num_specs):
  ...
  for key,value in specs_range.items():
      ...
      i+=1
  specs_range["max_interval_deviation"] = tuple(map(sum, zip(*[specs_range["mean_interval_deviation"], specs_range["max_interval_deviation"]])))
...
def main():
  ...
  CIR_YAML = "eval_engines/ae/ae_inputs/yaml_files/simple_example.yaml"
  
  gen_data(CIR_YAML, "simple_example", int(args.num_specs))
